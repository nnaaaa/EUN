...
export const useContent = <T = DefaultReturnType>(inputMessage: RefObject<null | HTMLInputElement>) => {
    ...
    const getContentAndImages = () => {
        ...
        return new Promise<Partial<T>>(resolve=>resolve(message))
    }
    const clearImages = () => {
    ...
    return { getContentAndImages, content, setContent,inputImages,previewImages,clearImages }
